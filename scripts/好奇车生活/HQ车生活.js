...
function RT(X, Y) {
    let rt;
    do {
        rt = Math.round(Math.random() * Y);
    } while (rt < X);
    console.log(`随机等待${rt}ms`);
    return rt;
}
const name = "好奇车生活"
const $ = new Env(name)
...
const { pinzanProxy } = require('./pinzan_proxy'); // 导入生成代理的函数
let accountId = ''
let openId = ''
let phone = ''
let notice = ''
...
async function main() {
    for (const item of accounts) {
        ...
        let user = await commonGet(`/cherycar/getCarUser?openId=${openId}`)
        if (user.success) {
            saveResultToFile("success", name)
            nickName = user.result.userName
            phoneNum = user.result.phone
            phone = phoneNum
            ...
            console.log("————————————")
            console.log("每日抽奖")
            let luckydraw = await luckyDrawGet('/luckydraw/luckydraw/D9AFF6F6BDE24C90AFED2ED1A3F3C3A0')
            if (luckydraw.success) {
                console.log(luckydraw.result.message)
                if (luckydraw.result.result) {
                    let luckydrawResult = await luckyDrawGet('/luckydraw/luckydrawResult/D9AFF6F6BDE24C90AFED2ED1A3F3C3A0')
                    if (luckydrawResult.result.result == "true") {
                        console.log(`获得：${luckydrawResult.result.awardName}`)
                    } else {
                        console.log(luckydrawResult.result.result)
            ...
            console.log(`账号: ${phone} | 拥有积分：${point.result}\n`)
            notice += `用户：${phone} 拥有积分: ${point.result}\n`
        } else {
